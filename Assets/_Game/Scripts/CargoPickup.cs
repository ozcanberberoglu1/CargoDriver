...
public class CargoPickup : MonoBehaviourPun, IPunObservable
{
    ...
    private void StopGrab()
    {
        if (heldRb != null)
        {
            ...
            // Same behaviour in both scenes: hand the box back to its own
            // PhotonTransformView. It is a networked object owned by the master,
            // so the master resumes physics and syncs to everyone. CargoAutoParent
            // re-attaches it to the truck once it rests (or it falls off if thrown).
